...
fn sum_of_parts_next_to_symbol(symbol_coordinate: TableCoordinate, table: &Vec<Vec<char>>) -> i32 {
    ...
    let mut number_of_numbers: i32 = 0;
    let mut numbers: [i32; 2] = [0, 0];
    for (dx, dy) in directions.iter() {
        let new_x: isize = symbol_coordinate.x as isize + dx;
        let new_y: isize = symbol_coordinate.y as isize + dy;
        ...
        if let Ok(num) = num_str.parse::<i32>() {
            if number_of_numbers > 1 {
                return 0; //return 0 if there are more than one numbers by the '*', as that means it is not a gear
            }
            numbers[number_of_numbers as usize] = num;
            number_of_numbers += 1;
        }
    }
    if number_of_numbers != 2 {
        return 0; //Default to 0 if there is not 2 adjacent numbers
    }
    numbers[0] * numbers[1]
}
